...
const Eobot = {
	eobotArr : [],
	api: 'https://www.eobot.com/api.aspx?',
	selectedIndex : -1,
	...
	sellPrice: {
		from: (currentCoinAmount, currentCoinPrice, fee, FutureAmount) => {
			const feeFinal = 1 - (fee / 100);
			return ((currentCoinAmount * currentCoinPrice) / (FutureAmount / feeFinal)).toFixed(4);
		},
		to:(currentCoinAmount, currentCoinPrice, fee, FutureAmount) => {
			const feeFinal = 1 - (fee / 100);
			return (((FutureAmount / feeFinal) * currentCoinPrice) / currentCoinAmount).toFixed(4);
		},
	},
	...
	mountCoins: async () => {
		try{
			...
			for (const transation of filtered) {
				const {	fromCoin,
					...
					const sellPriceFrom = Eobot.sellPrice['from'](toAmount, currentPrice[toCoin], fee, fromAmount);
					const sellPriceTo = Eobot.sellPrice['to'](toAmount, currentPrice[fromCoin], fee, fromAmount);
					...
					<h2 class="coin__title" title="From coin">${fromCoin}</h2>
					<span class="prices prices--amount"  title="${fromCoin} Amount you used to buy ${toCoin}">${fromCoin} ${fromAmount}</span>
					<span class="prices prices--amountprice" title="Amount price in $ when you bought ${toCoin}"><i class="fas fa-gem prices__icon"></i> $ ${parseFloat(fromAmountPrice).toFixed(8)}</span>
					<span class="prices prices--total" title="${fromCoin} Price when you bought"><i class="fas fa-bolt prices__icon"></i> $ ${fromPrice}</span>
					<span class="prices prices--sellprice" title="with ${toCoin} current price,  ${fromCoin} should be">Sell Price: $ ${sellPriceFrom}</span>
					</div>
					</div>
					<div class="coin coin--to">
					<div class="coin__img"><img class="coin__icon" src="${toImg}" alt="${toCoin}"></div>
					<div class="coin__details">
					<h2 class="coin__title" title="To coin">${toCoin}</h2>
					<span class="prices prices--amount" title="${toCoin} Amount you bought">${toCoin} ${toAmount}</span>
					<span class="prices prices--amountprice" title="${toCoin} Amount price when you bought">$ ${parseFloat(toAmountPrice).toFixed(8)}</span>
					<span class="prices prices--total" title="${toCoin} Price when you bought" >$ ${toPrice}</span>
					<span class="prices prices--sellprice" title="with ${fromCoin} current price,  ${toCoin} should be">Sell Price: $ ${sellPriceTo}</span>
					</div>
					</div>
					</div>
		...
		calcTransation: async(fromCoin, fromAmount, toCoin) => {
			try{
				const response = await fetch(`${Eobot.api}exchangefee=true&convertfrom=${fromCoin}&amount=${fromAmount}&convertto=${toCoin}&json=true`);
		...
		*/
		getCoinPrice: async (coins = []) => {
			let response = await fetch(`${Eobot.api}supportedcoins=true&currency=USD&json=true`);
